...
public class CWindowCentrality implements Centrality
{
	...
	protected int[][] Connectionchange=new int[36][36];
	...
	public double getGlobalCentrality(Map<DTNHost, List<Duration>> connHistory,int number)
	{
	  if(number==0){
		int[][] Temp1=new int[36][36];
		int[][] Temp2=new int[36][36];
		int[][] Temp3=new int[36][36];
		int all=0;
		for(int i=0;i<36;i++){
			for(int j=0;j<36;j++)
				for(int k=0;k<36;k++)
					Temp1[i][j]+=Connectionchange[i][k]*Connectionchange[k][j];
		}
		for(int i=0;i<36;i++){
			for(int j=0;j<36;j++)
		          if(Connectionchange[i][j]==0)
		        	  Temp2[i][j]=1;
		          else
		        	  Temp2[i][j]=0;
		}
	...
	public void setupConnectionchange(DTNHost thisHost, DTNHost otherHost){
		String T=thisHost.toString();
		String O=otherHost.toString();
		String Ts=T.substring(1);
		String Os=O.substring(1);
		int Ti=Integer.parseInt(Ts);
		int Oi=Integer.parseInt(Os);
		Connectionchange[Ti][Oi]=1;
		Connectionchange[Oi][Ti]=1;
		
		
	}
    public void setdownConnectionchange(DTNHost thisHost, DTNHost otherHost){
    	String T=thisHost.toString();
		String O=otherHost.toString();
		String Ts=T.substring(1);
		String Os=O.substring(1);
		int Ti=Integer.parseInt(Ts);
		int Oi=Integer.parseInt(Os);
		Connectionchange[Ti][Oi]=0;
		Connectionchange[Oi][Ti]=0;
		
		
	}
	public int getConnectionchange(DTNHost thisHost, DTNHost otherHost){
		String T=thisHost.toString();
		String O=otherHost.toString();
		String Ts=T.substring(1);
		String Os=O.substring(1);
