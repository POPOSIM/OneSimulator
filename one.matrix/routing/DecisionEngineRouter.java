 ...
 public int receiveMessage(Message m, DTNHost from){
	...
	public Message messageTransferred(String id, DTNHost from)
	{
		...
		if (outgoing!=null && decider.shouldSaveReceivedMessage(aMessage, getHost())) 
		{
			...
			//System.out.println(getMessageCollection()+"++++++"+getHost());
		...
		if (isFirstDelivery)
		{
			...
			//System.out.println(deliveredMessages+"9++++");
			//System.out.println(getMessageCollection()+"++++++"+getHost()+"+++"+SimClock.getTime());
 ...
 public void update(){
      ...
      /*if (!canStartTransfer() || isTransferring()) {
  		...
  		for(Connection c : getConnections()){
  			...
  			for(Message m : msgs)
  			{
  				if(decider.shouldSendMessageToHost(m, other)&&!(!canStartTransfer() || isTransferring())){
  					startTransfer(m,c);
