...
public class DecisionEngineRouter extends ActiveRouter
{
 ...
 public int receiveMessage(Message m, DTNHost from){
	...
	public Message messageTransferred(String id, DTNHost from)
	{
		...
		if (isFirstDelivery)
		{
			...
			//if(id.equals("Y3"))
		    System.out.println("1 "+from+" "+getHost());
			//System.out.println(deliveredMessages+"++++"+id+"+++++"+SimClock.getTime());
	...
	protected void transferDone(Connection con)
	{
		...
		i.hasNext();)
		{
			...
			Tuple<Message, Connection> t = i.next();
			//System.out.println(t.getKey().getId()+"+"+transferred.getId());
 ...
 public void update(){
	...
	/*private void SearchAllOutingMessage() {
		for(Connection c : getConnections()){
			Collection<Message> msgs = getMessageCollection();
			DTNHost other = c.getOtherNode(getHost());
			for(Message m : msgs)
			{
				if(decider.shouldSendMessageToHost(m, other)){
